...
/**
 * Block-cooperative CRC32C: all threads participate in data loading,
 * thread 0 processes CRC serially from shared memory.
 *
 * Uses tiled cooperative loads: all threads load a tile of data from global
 * memory into shared memory using coalesced uint4 reads, then thread 0
 * processes the tile from shared memory (~10× lower latency than global).
 *
 * @param data       GPU data pointer
 * @param len        Data length in bytes
 * @param shmem_tile Pointer to dynamic shared memory for tiling
 * @param tile_size  Size of the shared memory tile in bytes
                                           ...
                                           uint8_t *shmem_tile, size_t tile_size)
{
    ...
    size_t offset = 0;
    while (offset < len) {
        size_t this_tile = tile_size;
        if (offset + this_tile > len) this_tile = len - offset;

        /* ── All threads: coalesced load from global → shared memory ─── */
        const uint8_t *src = data + offset;
        size_t n16 = this_tile / 16;
        if (((uintptr_t)src & 0xF) == 0) {
            for (size_t i = tid; i < n16; i += nthreads)
                ((uint4 *)shmem_tile)[i] = ((const uint4 *)src)[i];
        } else {
            /* Unaligned: byte-level load across threads */
            for (size_t i = tid; i < n16 * 16; i += nthreads)
                shmem_tile[i] = src[i];
        }
        /* Tail bytes */
        for (size_t i = n16 * 16 + tid; i < this_tile; i += nthreads)
            shmem_tile[i] = src[i];
        __syncthreads();

        /* ── Thread 0: serial CRC from shared memory (fast access) ─── */
        if (tid == 0) {
            size_t j;
            for (j = 0; j + 16 <= this_tile; j += 16) {
                uint4 val = ((uint4 *)shmem_tile)[j / 16];
                uint8_t *bytes = (uint8_t *)&val;
                #pragma unroll
                for (int k = 0; k < 16; k++) {
                    uint8_t idx = (uint8_t)((crc ^ bytes[k]) & 0xFF);
            ...
            /* Tail bytes of this tile */
            for (; j < this_tile; j++) {
                uint8_t idx = (uint8_t)((crc ^ shmem_tile[j]) & 0xFF);
                crc = (crc >> 8) ^ crc32c_table[idx];
            }
        }
        __syncthreads();
...
static const uint32_t cpu_sha256_k[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
...
static inline uint32_t cpu_rotr(uint32_t x, int n) { return (x >> n) | (x << (32 - n)); }
...
void cpu_sha256(const void *data, size_t len, uint8_t hash_out[32])
{
    ...
    for (size_t block = 0; block < padded; block += 64) {
        ...
        for (int i = 16; i < 64; i++) {
            uint32_t s0 = cpu_rotr(w[i-15],7) ^ cpu_rotr(w[i-15],18) ^ (w[i-15] >> 3);
            uint32_t s1 = cpu_rotr(w[i-2],17) ^ cpu_rotr(w[i-2],19) ^ (w[i-2] >> 10);
            w[i] = s1 + w[i-7] + s0 + w[i-16];
        }

        uint32_t a=h0, b=h1, c=h2, d=h3, e=h4, f=h5, g=h6, hh=h7;
        for (int i = 0; i < 64; i++) {
            uint32_t sig1 = cpu_rotr(e,6) ^ cpu_rotr(e,11) ^ cpu_rotr(e,25);
            uint32_t ch = (e & f) ^ (~e & g);
            uint32_t t1 = hh + sig1 + ch + cpu_sha256_k[i] + w[i];
            uint32_t sig0 = cpu_rotr(a,2) ^ cpu_rotr(a,13) ^ cpu_rotr(a,22);
